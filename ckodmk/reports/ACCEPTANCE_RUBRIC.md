...
| External evaluator handoff | Deterministic private archive requires both retained labs, both locks, all SBOMs and raw evidence; `AUDITOR_HANDOFF.md` defines clean execution and a signed return package; `verify_external_campaign_preregistration.py` rejects author-controlled, undersized, unmatched-budget, denominator-laundered, or prematurely unblinded plans and can authenticate evaluator identity with OpenSSH; `verify_external_campaign_result.py` recomputes quotas, ordered denominators, false passes, controls, seed opening, baseline presence, and matched budgets from terminal per-case records; `verify_external_evaluation.py` binds the signed preregistration, terminal manifest, seed reveal, and raw evidence while recomputing the minimum return profile without claiming to open referenced evidence objects |
| Governance | `GOVERNANCE.md` defines schema evolution, support, vulnerability handling, release authority, signing and revocation requirements, license/data governance, retention, and change control; repository-level immutable releases are enabled for future publications, while MFENX owner-root signing and protected-branch/private workflow-attestation enforcement remain unavailable or unimplemented |
