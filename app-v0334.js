...
const el = Object.fromEntries(
  [
    ...
    "portal-result-kind",
    "portal-result-title",
    "portal-result-detail",
    "portal-rootid",
    "portal-replay",
    "portal-input-kind",
    "portal-input-bytes",
    "portal-copy-rootid",
    "portal-open-certificate",
    "portal-drop-zone",
    "hero-run-button",
    "hero-run-label",
    "hero-portal-button",
    ...
    "sfcs-widget-branch",
    "sfcs-widget-replay",
    "sfcs-widget-progress",
    "sfcs-lane-bitwise",
    "sfcs-lane-bitwise-status",
    "sfcs-lane-compare",
    "sfcs-lane-compare-status",
    "sfcs-lane-branch",
    "sfcs-lane-branch-status",
    "sfcs-lane-memory",
    "sfcs-lane-memory-status",
    "sfcs-range-proof-count",
    "sfcs-byte-semantics-count",
    "sfcs-mutation-count",
...
function cameraHorizontalOffset() {
  if (isCompactLandscape()) return -0.42;
  if (window.innerWidth <= 900) return 0;
  if (window.innerWidth <= 1240) return -0.46;
  return -0.68;
}
...
const selectedCityRipples = [];
const labelWorldPosition = new THREE.Vector3();
const labelProjectedPosition = new THREE.Vector3();
const labelSurfaceNormal = new THREE.Vector3();
const labelViewVector = new THREE.Vector3();
const orbitScratchPosition = new THREE.Vector3();
let proofOrbitLabel;
let cloudLayer;
let cloudMaterial;
let proofConstellation;
let proofConstellationMaterial;
...
function updateAstronomy() {
  ...
  if (cloudMaterial) {
    cloudMaterial.uniforms.sunDirection.value.copy(sunVector);
  }
  if (atmosphereMaterial) {
    atmosphereMaterial.uniforms.sunDirection.value.copy(sunVector);
  }
...
function createEarthGrid() {
  ...
  for (let lat = -60; lat <= 60; lat += 30) {
    ...
    group.add(
      new THREE.Line(
        ...
        lat === 0 ? majorMaterial : minorMaterial,
      ),
    );
  }
  for (let lon = -150; lon <= 180; lon += 30) {
    ...
    group.add(
      new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(points),
        lon === 0 || Math.abs(lon) === 90 ? majorMaterial : minorMaterial,
      ),
    );
  }
  return group;
  ...
  pairs.forEach(([fromIndex, toIndex], index) => {
    ...
    const glow =
      window.innerWidth <= 760
        ? null
        : new THREE.Mesh(
            new THREE.TubeGeometry(curve, 96, 0.008, 5, false),
            new THREE.MeshBasicMaterial({
              color: index === 1 ? 0xffc15a : 0x45ddd2,
              transparent: true,
              opacity: 0.075,
    ...
    if (glow) networkGroup.add(glow);
    networkGroup.add(line, pulse);
    networkLinks.push({ curve, line, glow, pulse, phase: index / pairs.length });
  });
...
function createSelectedCityGeometry() {
  ...
  for (let index = 0; index < 3; index += 1) {
    const ripple = new THREE.Mesh(
      new THREE.RingGeometry(0.045, 0.052, 48),
      new THREE.MeshBasicMaterial({
        color: index === 1 ? 0x45ddd2 : 0xb9ff3d,
        transparent: true,
        opacity: 0,
        side: THREE.DoubleSide,
        blending: THREE.AdditiveBlending,
    ...
    selectedCityRipples.push(ripple);
    earthGroup.add(ripple);
  }
...
function updateSelectedCityGeometry() {
  ...
  selectedCityRipples.forEach((ripple) => {
    ripple.position.copy(surface);
    ripple.quaternion.copy(selectedCityHalo.quaternion);
  });
...
function createOrbitalLabels() {
  el.orbitalLabelLayer.replaceChildren();
  orbitalLabelEntries.length = 0;
  cities.forEach((city, index) => {
    const label = document.createElement("div");
    label.className = "orbital-map-label";
    const reticle = document.createElement("i");
    const copy = document.createElement("span");
    const code = document.createElement("b");
    const role = document.createElement("small");
    code.textContent = city.code;
    role.textContent = networkCityIndexes.includes(index) ? "VALIDATOR" : "CITY";
    copy.append(code, role);
    label.append(reticle, copy);
    label.hidden = true;
    el.orbitalLabelLayer.append(label);
    orbitalLabelEntries.push({ index, label, role });
  });

  proofOrbitLabel = document.createElement("div");
  proofOrbitLabel.className = "orbital-map-label proof-orbit";
  const proofSignal = document.createElement("i");
  const proofCopy = document.createElement("span");
  const proofName = document.createElement("b");
  const proofState = document.createElement("small");
  proofCopy.append(proofName, proofState);
  proofOrbitLabel.append(proofSignal, proofCopy);
  proofOrbitLabel.hidden = true;
  proofOrbitLabel.userData = { proofName, proofState };
  el.orbitalLabelLayer.append(proofOrbitLabel);
}

function positionOrbitalLabel(
  label,
  worldPosition,
  visible,
  canvasBounds,
  ...
  labelProjectedPosition.copy(worldPosition).project(camera);
  const inFrame =
    labelProjectedPosition.z > -1 &&
    labelProjectedPosition.z < 1 &&
    Math.abs(labelProjectedPosition.x) < 0.96 &&
    Math.abs(labelProjectedPosition.y) < 0.92;
  if (!inFrame) {
    label.hidden = true;
    label.classList.remove("visible");
    return;
  ...
  const x =
    canvasBounds.left -
    layerBounds.left +
    (labelProjectedPosition.x * 0.5 + 0.5) * canvasBounds.width;
  const y =
    canvasBounds.top -
    layerBounds.top +
    (-labelProjectedPosition.y * 0.5 + 0.5) * canvasBounds.height;
  label.hidden = false;
  label.style.transform = `translate3d(${Math.round(x)}px, ${Math.round(y)}px, 0) translate(-50%, -50%)`;
  label.classList.add("visible");
}

function updateOrbitalLabels() {
  if (!earthGroup || !camera || !el.orbitalLabelLayer) return;
  const canvasBounds = el.canvas.getBoundingClientRect();
  const layerBounds = el.orbitalLabelLayer.getBoundingClientRect();
  orbitalLabelEntries.forEach(({ index, label, role }) => {
    const selected = index === state.activeCity;
    const relevant = selected || networkCityIndexes.includes(index);
    label.classList.toggle("selected", selected);
    const roleText = selected ? "SELECTED" : "VALIDATOR";
    if (role.textContent !== roleText) role.textContent = roleText;
    cityMarkers[index].getWorldPosition(labelWorldPosition);
    labelSurfaceNormal.copy(labelWorldPosition).normalize();
    labelViewVector.copy(camera.position).sub(labelWorldPosition).normalize();
    const facingCamera = labelSurfaceNormal.dot(labelViewVector) > 0.08;
    positionOrbitalLabel(
      label,
      labelWorldPosition,
      relevant && facingCamera,
      canvasBounds,
      layerBounds,
    );
  });

  const selectedOrbit = orbitEntries.find((entry) => entry.name === state.mode);
  if (!selectedOrbit || !proofOrbitLabel) return;
  selectedOrbit.beacon.getWorldPosition(labelWorldPosition);
  const proofName = state.mode.toUpperCase();
  const proofState = state.running
    ? `VERIFYING ${Math.round(state.proofProgress * 100)}%`
    : state.lastResult
      ? "CERTIFICATE ACCEPTED"
      : "SELECTED PROOF ORBIT";
  if (proofOrbitLabel.userData.proofName.textContent !== proofName) {
    proofOrbitLabel.userData.proofName.textContent = proofName;
  }
  if (proofOrbitLabel.userData.proofState.textContent !== proofState) {
    proofOrbitLabel.userData.proofState.textContent = proofState;
  }
  positionOrbitalLabel(
    proofOrbitLabel,
    labelWorldPosition,
...
function cloudHash(x, y, seed) {
  let value =
    ...
    Math.imul(y + seed * 1999, 668265263);
  value = Math.imul(value ^ (value >>> 13), 1274126177);
  return ((value ^ (value >>> 16)) >>> 0) / 4294967295;
}

function cloudNoise(u, v, frequency, seed) {
  const px = u * frequency;
  const py = v * frequency * 0.55;
  ...
  const wrapX = (value) => ((value % frequency) + frequency) % frequency;
  const top = THREE.MathUtils.lerp(
    cloudHash(wrapX(x0), y0, seed),
    cloudHash(wrapX(x1), y0, seed),
    tx,
  );
  const bottom = THREE.MathUtils.lerp(
    cloudHash(wrapX(x0), y1, seed),
    cloudHash(wrapX(x1), y1, seed),
    tx,
  );
  return THREE.MathUtils.lerp(top, bottom, ty);
}

function createCloudTexture(mobile) {
  const width = mobile ? 192 : 384;
  const height = mobile ? 96 : 192;
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      const u = x / width;
      const broad = cloudNoise(u, v, 5, 7) * 0.56;
      const middle = cloudNoise(u, v, 11, 17) * 0.29;
      const detail = cloudNoise(u, v, 23, 29) * 0.15;
      const belt = Math.sin((v * 5.4 + u * 0.8) * Math.PI) * 0.035;
      const density = THREE.MathUtils.smoothstep(
        broad + middle + detail + belt - latitude * 0.035,
        0.54,
        0.73,
      );
...
function createCloudLayer(mobile) {
  cloudMaterial = new THREE.ShaderMaterial({
    uniforms: {
      cloudMap: { value: createCloudTexture(mobile) },
      sunDirection: { value: new THREE.Vector3(1, 0, 0) },
      cloudOpacity: { value: mobile ? 0.58 : 0.82 },
    },
    transparent: true,
    depthWrite: false,
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float daylight = smoothstep(-0.16, 0.28, solar);
        vec3 viewDirection = normalize(-vViewPosition);
        float horizon = pow(1.0 - max(dot(normalize(vViewNormal), viewDirection), 0.0), 2.2);
        vec3 shadowColor = vec3(0.08, 0.28, 0.31);
        vec3 daylightColor = vec3(0.94, 1.0, 0.97);
        vec3 color = mix(shadowColor, daylightColor, daylight);
        float alpha = cloud * (0.055 + daylight * 0.19 + horizon * 0.08) * cloudOpacity;
        gl_FragColor = vec4(color, alpha);
      }
    `,
  });
  cloudLayer = new THREE.Mesh(
    new THREE.SphereGeometry(EARTH_RADIUS + 0.018, mobile ? 56 : 96, mobile ? 36 : 64),
    cloudMaterial,
  );
  cloudLayer.rotation.y = 0.22;
...
async function createEarth() {
  ...
  earthMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        float solar = dot(normalize(vObjectNormal), normalize(sunDirection));
        float daylight = smoothstep(-0.2, 0.18, solar);
        vec3 day = texture2D(dayMap, vUv).rgb;
        vec3 nightSample = texture2D(nightMap, vUv).rgb;
        float dayLuma = dot(day, vec3(0.2126, 0.7152, 0.0722));
        day = mix(vec3(dayLuma), day, 1.16);
        day = pow(max(day, vec3(0.0)), vec3(0.9)) * 1.06;
        float cityLight = smoothstep(0.18, 0.78, max(nightSample.r, max(nightSample.g, nightSample.b)));
        vec3 night = nightSample * 1.75 + vec3(1.0, 0.47, 0.12) * cityLight * 0.22;
        vec3 litDay = day * (0.58 + max(solar, 0.0) * 0.88);
        vec3 viewDirection = normalize(-vViewPosition);
        float limb = pow(1.0 - max(dot(normalize(vViewNormal), viewDirection), 0.0), 2.6);
        float terminator = 1.0 - smoothstep(0.0, 0.2, abs(solar));
        float ocean = smoothstep(0.025, 0.16, day.b - max(day.r * 0.78, day.g * 0.58));
        float oceanSheen = ocean * pow(max(solar, 0.0), 3.0) * (1.0 - limb);
        vec3 color = mix(night, litDay, daylight);
        color += day * 0.045;
        color += vec3(0.05, 0.38, 0.36) * limb * 0.68;
        color += vec3(0.52, 0.25, 0.08) * terminator * 0.12;
        color += vec3(0.1, 0.34, 0.38) * oceanSheen * 0.28;
  ...
  atmosphereMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float solar = dot(normalize(vObjectNormal), normalize(sunDirection));
        float daylight = smoothstep(-0.24, 0.32, solar);
        float terminator = 1.0 - smoothstep(0.0, 0.22, abs(solar));
        vec3 atmosphere = mix(vec3(0.08, 0.54, 0.58), vec3(0.2, 0.91, 0.8), daylight);
        atmosphere = mix(atmosphere, vec3(1.0, 0.47, 0.13), terminator * 0.38);
        float pulse = 0.96 + sin(time * 0.42) * 0.04;
        float alpha = (fresnel * (0.18 + daylight * 0.78) + terminator * fresnel * 0.12) * strength * pulse;
        gl_FragColor = vec4(atmosphere, alpha);
      }
    `,
  });
  const outerAtmosphere = new THREE.Mesh(
    new THREE.SphereGeometry(EARTH_RADIUS + 0.12, mobile ? 48 : 72, mobile ? 32 : 48),
    atmosphereMaterial,
  );
...
function createOrbitTrack(name, index) {
  ...
  const trailPointCount = window.innerWidth <= 760 ? 12 : 26;
  ...
  const beaconRing = new THREE.Mesh(
    new THREE.TorusGeometry(0.061, 0.005, 6, 36),
    new THREE.MeshBasicMaterial({
      color: modes[name].color,
      transparent: true,
      opacity: 0.7,
  ...
  const beaconOuterRing = new THREE.Mesh(
    new THREE.TorusGeometry(0.092, 0.0025, 5, 48),
    new THREE.MeshBasicMaterial({
      color: modes[name].color,
      transparent: true,
      opacity: 0.26,
  ...
  beaconOuterRing.rotation.y = Math.PI / 2;
  beacon.add(beaconRing, beaconOuterRing);
  ...
  orbitGroup.add(line, trail, beacon);
  orbitEntries.push({ name, line, trail, beacon, beaconRing, beaconOuterRing });
...
function createProofField() {
  proofShellMaterial = new THREE.LineBasicMaterial({
  ...
  proofShell = new THREE.LineSegments(
    new THREE.EdgesGeometry(new THREE.IcosahedronGeometry(EARTH_RADIUS + 0.27, 2), 4),
    proofShellMaterial,
  );
  proofConstellationMaterial = new THREE.LineBasicMaterial({
    color: 0x45ddd2,
    transparent: true,
    opacity: 0.026,
    blending: THREE.AdditiveBlending,
  ...
  proofConstellation = new THREE.LineSegments(
    new THREE.EdgesGeometry(new THREE.IcosahedronGeometry(EARTH_RADIUS + 0.94, 1), 4),
    proofConstellationMaterial,
  );
  proofConstellation.rotation.set(-0.18, 0.34, 0.12);
...
function raycast(event) {
  ...
  return (
    raycaster.intersectObjects(interactiveObjects, false).find((hit) => {
      ...
      labelSurfaceNormal.copy(hitWorldPosition).normalize();
      labelViewVector.copy(camera.position).sub(hitWorldPosition).normalize();
      return labelSurfaceNormal.dot(labelViewVector) > 0.08;
    }) || null
  );
}
...
function animate(time = 0) {
  ...
  if (cloudLayer && cloudMaterial) {
    cloudLayer.rotation.y += state.motion ? 0.000035 : 0;
    cloudLayer.rotation.z = Math.sin(seconds * 0.012) * 0.006;
  }
  if (atmosphereMaterial) atmosphereMaterial.uniforms.time.value = seconds;
  if (orbitGroup) {
    orbitGroup.rotation.y = state.motion ? seconds * 0.022 : orbitGroup.rotation.y;
    orbitEntries.forEach(
      ({ name, trail, beacon, beaconRing, beaconOuterRing }, index) => {
        const data = beacon.userData;
        const angle = data.phase + seconds * (state.motion ? data.speed : 0);
        orbitScratchPosition.set(
          Math.cos(angle) * data.radius,
          0,
          Math.sin(angle) * data.radius,
        );
        orbitScratchPosition.applyEuler(data.rotation);
        beacon.position.copy(orbitScratchPosition);
        const trailAttribute = trail.geometry.attributes.position;
        for (let trailIndex = 0; trailIndex < data.trailPointCount; trailIndex += 1) {
          const trailAngle = angle - trailIndex * (0.027 + index * 0.0015);
          orbitScratchPosition
            .set(
              Math.cos(trailAngle) * data.radius,
              0,
              Math.sin(trailAngle) * data.radius,
            )
            .applyEuler(data.rotation);
          trailAttribute.setXYZ(
            trailIndex,
            orbitScratchPosition.x,
            orbitScratchPosition.y,
            orbitScratchPosition.z,
          );
        }
        ...
        beaconRing.rotation.z += state.motion ? 0.018 : 0;
        beaconOuterRing.rotation.x += state.motion ? -0.011 : 0;
        trail.material.opacity = selected
          ? 0.58 + state.proofProgress * 0.32
          : 0.055;
      },
    );
  }
  if (networkGroup) {
    networkLinks.forEach(({ curve, pulse, line, glow, phase }, index) => {
      ...
      if (glow) glow.material.opacity = 0.055 + Math.sin(seconds * 1.1 + index) * 0.018;
    });
  }
  ...
  if (selectedCityHalo) {
    ...
    selectedCityRipples.forEach((ripple) => {
      const phase = (seconds * 0.34 + ripple.userData.phase) % 1;
      ripple.scale.setScalar(0.8 + phase * 2.5);
      ripple.material.opacity = (1 - phase) * (state.running ? 0.5 : 0.24);
    });
  }
  if (subsolarMarker) {
  ...
  if (proofShell && proofRingGroup) {
    ...
    if (proofConstellation) {
      proofConstellation.rotation.y = seconds * -0.009;
      proofConstellation.rotation.x = -0.18 + Math.sin(seconds * 0.08) * 0.035;
      proofConstellationMaterial.color.setHex(modes[state.mode].color);
...
function updateOrbitSelection() {
  orbitEntries.forEach(({ name, line, trail, beacon, beaconRing, beaconOuterRing }) => {
    const selected = name === state.mode;
    line.material.opacity = selected ? 0.52 : 0.055;
    trail.material.opacity = selected ? 0.88 : 0.08;
    beacon.material.opacity = selected ? 1 : 0.42;
    beaconRing.material.opacity = selected ? 0.82 : 0.2;
    beaconOuterRing.material.opacity = selected ? 0.34 : 0.08;
  });
}
...
function selectMode(name, withTone = true) {
  ...
  el.heroRunLabel.textContent = mode.button;
  el.heroRunButton.disabled = false;
  el.heroRunButton.dataset.state = "ready";
  el.heroLocalState.textContent = "BROWSER LOCAL";
  el.heroUploadState.textContent = "NONE";
  el.heroMeaningState.textContent =
    name === "rootprint" ? "SLBIT BOUND / NON-CORE" : "OPTIONAL / NON-CORE";
...
function beginRun() {
  ...
  el.heroRunButton.disabled = true;
  el.heroRunButton.dataset.state = "running";
  el.heroLocalState.textContent = "VERIFYING";
  el.shareButton.disabled = true;
  el.statusSeal.classList.remove("verified");
...
function completeRun(digest, claim = "VERIFIED") {
  ...
  el.heroRunButton.disabled = false;
  el.heroRunButton.dataset.state = "valid";
  el.heroLocalState.textContent = "ACCEPTED";
...
function shortProofId(value, head = 12, tail = 8) {
  const text = String(value || "");
  if (!text) return "PENDING";
  if (text.length <= head + tail + 3) return text.toUpperCase();
  return `${text.slice(0, head)}...${text.slice(-tail)}`.toUpperCase();
}

const sfcsLaneElements = {
  bitwise: ["sfcsLaneBitwise", "sfcsLaneBitwiseStatus"],
  compare: ["sfcsLaneCompare", "sfcsLaneCompareStatus"],
  branch: ["sfcsLaneBranch", "sfcsLaneBranchStatus"],
  memory: ["sfcsLaneMemory", "sfcsLaneMemoryStatus"],
};

function setSfcsCoverageLanes(lanes = {}) {
  Object.entries(sfcsLaneElements).forEach(([key, [laneKey, statusKey]]) => {
    const lane = el[laneKey];
    if (!lane) return;
    const update = lanes[key] || {};
    const stateName = update.state || "idle";
    lane.dataset.state = stateName;
    lane.style.setProperty("--lane-progress", `${Math.max(0, Math.min(100, Number(update.progress) || 0))}%`);
    if (el[statusKey]) el[statusKey].textContent = update.label || "STANDBY";
  });
}

function setSfcsWidgetState(status, data = {}) {
  if (!el.sfcsRunConsole) return;
  ...
  el.sfcsWidgetReplay.textContent = data.replay ? shortProofId(data.replay) : "PENDING";
  el.sfcsWidgetProgress.style.width = `${Math.max(0, Math.min(100, Number(data.progress) || 0))}%`;
  if (data.lanes) setSfcsCoverageLanes(data.lanes);
  if (data.rangeProofs && el.sfcsRangeProofCount) el.sfcsRangeProofCount.textContent = data.rangeProofs;
  if (data.byteSemantics && el.sfcsByteSemanticsCount) el.sfcsByteSemanticsCount.textContent = data.byteSemantics;
...
async function runSfcsFractal() {
  ...
  try {
    ...
    setSfcsWidgetState("running", {
      statusLabel: "RUNNING",
      title: "Graph accepted",
      detail: `${report.fastPathNodes.length} structured nodes routed to the Sovereign Fast Path; ${report.denseNodes.length} dense nodes retained for synthesis.`,
      stage: "TRACE",
    ...
    setSfcsWidgetState("running", {
      statusLabel: "RUNNING",
      title: "Execution trace replayed",
      detail: `${trace.steps.length} deterministic VM/fractal trace steps accepted with output decision ${trace.outputs.decision}.`,
      stage: "SYNTHESIS",
    ...
    setSfcsWidgetState("running", {
      statusLabel: "RUNNING",
      title: "Synthesis plan committed",
      detail: `${synthesis.operations.length} synthesis operations recorded with the embedding invariant preserved.`,
      stage: ".PHA",
    ...
    setSfcsWidgetState("running", {
      statusLabel: "RUNNING",
      title: ".PHA identity derived",
      detail: "Core fingerprint binds graph, trace, synthesis, provenance, and public execution state.",
      stage: "ROOTPRINT",
      digest: phx,
      progress: 72,
      lanes: {
    ...
    setSfcsWidgetState("running", {
      statusLabel: "RUNNING",
      title: "Rootprint replay bound",
      detail: "Branch ID and replay fingerprint are deterministic and bound to the .pha core identity.",
      stage: "FINAL",
      digest: phx,
      branch: branchId,
      ...
      progress: 90,
      lanes: {
        bitwise: { state: "valid", progress: 100, label: "ROOTPRINT BOUND" },
        compare: { state: "valid", progress: 100, label: "ROOTPRINT BOUND" },
    ...
    setSfcsWidgetState("invalid", {
      ...
      progress: 100,
      lanes: {
        bitwise: { state: "invalid", progress: 100, label: "INSPECT" },
        compare: { state: "invalid", progress: 100, label: "INSPECT" },
        branch: { state: "invalid", progress: 100, label: "INSPECT" },
        memory: { state: "invalid", progress: 100, label: "INSPECT" },
      },
      mutations: "REJECTED",
    });
...
function failRun(message) {
  ...
  el.verifyButton.disabled = false;
  el.heroRunButton.disabled = false;
  el.heroRunButton.dataset.state = "invalid";
  el.heroLocalState.textContent = "REJECTED";
...
function setPortalOpen(open) {
  if (open) {
    ...
    el.sfcsOrbitToggle.setAttribute("aria-expanded", "false");
  }
...
function classifyPortalInput(raw) {
  const trimmed = raw.trim();
  const rootIdPattern =
    /^(rootid|seed)[:/_-]\S+|^(rootid[:/_-])?[a-f0-9]{32,}|^(phx|pha|phm|root|branch|replay|sha256):[a-f0-9]{16,}/i;
  if (rootIdPattern.test(trimmed)) {
    return {
      kind: "rootid-seed",
      label: "ROOTID SEED",
      normalized: trimmed.toLowerCase(),
      detail:
        "The browser interpreted this as an identity or digest seed and produced a deterministic local RootID observation.",
    };
  }

  const webUrlLike = /^[a-z][a-z\d+.-]*:\/\//i.test(trimmed);
  const bareDomainLike = /^(www\.)?[a-z0-9-]+(\.[a-z0-9-]+)+(\/\S*)?$/i.test(trimmed);
  try {
    if (!webUrlLike && !bareDomainLike) throw new Error("not a web URL");
    const parsed = new URL(webUrlLike ? trimmed : `https://${trimmed}`);
    return {
      kind: "url",
      label: "URL",
      normalized: parsed.href,
      detail:
        "The browser fingerprinted the normalized URL string locally. It did not crawl the URL or claim remote content truth.",
    };
  } catch {
    const looksLikeClaim =
      trimmed.length <= 180 &&
      /^(claim:|prove:|verify:|rootid:|[a-z0-9][^{}[\]\n]{8,})/i.test(trimmed);
    return {
      kind: looksLikeClaim ? "claim" : "content",
      label: looksLikeClaim ? "CLAIM" : "TEXT",
      normalized: trimmed,
      detail:
        "The browser fingerprinted the pasted content locally and created a deterministic RootID observation without uploading it.",
    };
  }
}

function shortenDigest(value, head = 18, tail = 10) {
  const text = String(value || "");
  if (text.length <= head + tail + 3) return text.toUpperCase();
  return `${text.slice(0, head)}...${text.slice(-tail)}`.toUpperCase();
}

function setPortalResult(status, result = {}) {
  el.portalResult.dataset.status = status;
  el.portalResultKind.textContent = result.kindLabel || "LOCAL ONLY";
  el.portalResultTitle.textContent = result.title || "READY FOR LOCAL VERIFICATION";
  el.portalResultDetail.textContent =
    result.detail ||
    "The verifier runs in this browser. It creates deterministic RootID and replay fingerprints for text, URLs, claims, and RootID seeds without uploading proof data by default.";
  el.portalRootid.textContent = result.rootId ? shortenDigest(result.rootId, 22, 12) : "WAITING";
  el.portalReplay.textContent = result.replay ? shortenDigest(result.replay, 22, 12) : "WAITING";
  el.portalInputKind.textContent = result.inputKind || "WAITING";
  el.portalInputBytes.textContent = Number(result.bytes || 0).toLocaleString("en-US");
  const ready = status === "valid" && Boolean(result.rootId);
  el.portalCopyRootid.disabled = !ready;
  el.portalOpenCertificate.disabled = !ready;
  state.portalLastResult = ready ? result : null;
}

async function verifyPortalInput() {
  const raw = el.portalInput.value.trim();
  if (!raw) {
    setPortalResult("warning", {
      kindLabel: "INPUT REQUIRED",
      title: "PASTE TEXT, A URL, A CLAIM, OR A ROOTID SEED",
      detail: "Nothing was verified because the local verifier received an empty input.",
    });
  ...
  beginRun();
  try {
    setPortalResult("running", {
      kindLabel: "LOCAL VERIFY",
      title: "DERIVING DETERMINISTIC ROOTID",
      detail: "Canonicalizing the input and calculating local RootID and replay fingerprints.",
      inputKind: "RUNNING",
    });

    const classified = classifyPortalInput(raw);
    const bytes = new TextEncoder().encode(classified.normalized).length;

    const payload = {
      schema: "mfenx/rootid-local-verification/v1",
      kind: classified.kind,
      value_digest: await sha256Hex(classified.normalized),
      value_length: bytes,
    ...
    el.verificationDetail.textContent = classified.detail;
    ...
    setPortalResult("valid", {
      kindLabel: classified.label,
      title: "LOCAL ROOTID CREATED",
      detail: `${classified.detail} Core identity is deterministic; no proof data was uploaded by this browser verifier.`,
      rootId,
      replay,
      inputKind: classified.label,
      bytes,
      normalized: classified.normalized,
      valueDigest: payload.value_digest,
    });
    completeRun(rootId.slice(7), "ROOTID LOCAL OK");
  } catch (error) {
    setPortalResult("invalid", {
      kindLabel: "REJECTED",
...
function bindInterface() {
  ...
  el.heroRunButton.addEventListener("click", () => {
    if (state.running) return;
    if (state.mode === "sfcs") el.sfcsOrbitRun.click();
    else modes[state.mode].action();
  });
  el.heroPortalButton.addEventListener("click", () => setPortalOpen(true));
  ...
  el.portalInputClear.addEventListener("click", () => {
    ...
    setPortalResult("idle");
    el.portalInput.focus();
  });
  el.portalCopyRootid.addEventListener("click", () => {
    if (!state.portalLastResult?.rootId) return;
    copyText(state.portalLastResult.rootId, "Local RootID copied");
  });
  el.portalOpenCertificate.addEventListener("click", () => {
    if (!state.portalLastResult?.rootId) return;
    setPortalOpen(false);
    window.setTimeout(() => {
      el.verificationStatus.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 80);
  });
  ...
  for (const eventName of ["dragenter", "dragover"]) {
    el.portalDropZone.addEventListener(eventName, (event) => {
      event.preventDefault();
      el.portalDropZone.classList.add("dragging");
    });
  }
  for (const eventName of ["dragleave", "drop"]) {
    el.portalDropZone.addEventListener(eventName, (event) => {
      event.preventDefault();
      el.portalDropZone.classList.remove("dragging");
    });
  }
  el.portalDropZone.addEventListener("drop", (event) => {
    const files = event.dataTransfer?.files;
    if (files?.length) {
      setPortalOpen(false);
      verifyLocalArtifacts(files);
    }
  });
  document.querySelectorAll("[data-portal-mode]").forEach((button) => {
    button.addEventListener("click", () => {
      const mode = button.dataset.portalMode;
      if (!modes[mode] || state.running) return;
      setPortalOpen(false);
      selectMode(mode);
      modes[mode].action();
    });
  ...
  el.artifactInput.addEventListener("change", () => {
    if (el.artifactInput.files.length) {
      if (document.body.classList.contains("portal-open")) setPortalOpen(false);
      verifyLocalArtifacts(el.artifactInput.files);
    }
  });
  ...
  const setSfcsOrbitOpen = (open) => {
    if (open) {
      setPortalOpen(false);
      if (!state.running) selectMode("sfcs");
    }
  ...
  const activateSfcsWidget = (run = false) => {
  ...
  el.sfcsOrbitClose.addEventListener("click", (event) => {
    event.stopPropagation();
    setSfcsOrbitOpen(false);
  });
  el.sfcsOrbitRun.addEventListener("click", (event) => {
    event.stopPropagation();
    activateSfcsWidget(true);
  });
  ...
  window.addEventListener("hashchange", () => {
    if (window.location.hash === "#sfcs" || window.location.hash === "#sfcs-run") {
      activateSfcsWidget(window.location.hash === "#sfcs-run");
    }
  });
  if (window.location.hash === "#sfcs" || window.location.hash === "#sfcs-run") {
