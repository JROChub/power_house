...
(function (global) {
  ...
  const DEMOS = Object.freeze({
    mlp: Object.freeze({ source: "demo/source.onnx", sourceName: "source.onnx", contract: "demo/browser-contract.json", contractName: "browser-contract.json", contractDigest: "sha256:df48a3f723e5a8da61c3fe401172195f12c520a37e19a2bd4f5af03ee6f1c39e" }),
    rbf: Object.freeze({ source: "demo/rbf/source.onnx", sourceName: "rbf-source.onnx", contract: "demo/rbf/browser-contract.json", contractName: "rbf-browser-contract.json", contractDigest: "sha256:46a9e1c548198e5401afaee38f2c95bb4a6853d654fffeb62738d94cd3433f0e" })
  });
  async function loadDemo(autoRun = false) { const button = global.document.getElementById("load-demo"), demoButton = global.document.getElementById("run-demo"), run = global.document.getElementById("run-gate"), selected = global.document.getElementById("demo-model")?.value || "mlp", demo = DEMOS[selected]; if (!demo) { showDecision("BLOCK", "selected example is unsupported"); return; } button.disabled = true; if (demoButton) demoButton.disabled = true; run.disabled = true; showDecision("LOADING", "Loading the trained source, data set, and contract…"); try { const [source, dataset, contract] = await Promise.all([fetchFile(demo.source, demo.sourceName, "application/octet-stream"), fetchFile("demo/optdigits-official-test.npz", "optdigits-official-test.npz", "application/octet-stream"), fetchFile(demo.contract, demo.contractName, "application/json")]); setInputFile("gate-source", source); setInputFile("gate-dataset", dataset); setInputFile("gate-contract", contract); global.document.getElementById("gate-contract-digest").value = demo.contractDigest; await buildCandidate(source); showDecision("READY", autoRun ? "Candidate built. Starting local verification…" : "Example inputs loaded and candidate built. Select RUN LOCAL GATE."); if (autoRun) global.document.getElementById("gate-form").requestSubmit(); } catch (error) { showDecision("BLOCK", error.message); } finally { button.disabled = false; if (demoButton) demoButton.disabled = false; if (!autoRun) run.disabled = false; } }
