...
(function (global) {
  ...
  async function loadDemo(autoRun = false) { const button = global.document.getElementById("load-demo"), demo = global.document.getElementById("run-demo"), run = global.document.getElementById("run-gate"); button.disabled = true; if (demo) demo.disabled = true; run.disabled = true; showDecision("LOADING", "Loading the retained trained source, data set, and contract…"); try { const [source, dataset, contract] = await Promise.all([fetchFile("demo/source.onnx", "source.onnx", "application/octet-stream"), fetchFile("demo/optdigits-official-test.npz", "optdigits-official-test.npz", "application/octet-stream"), fetchFile("demo/browser-contract.json", "browser-contract.json", "application/json")]); setInputFile("gate-source", source); setInputFile("gate-dataset", dataset); setInputFile("gate-contract", contract); global.document.getElementById("gate-contract-digest").value = "sha256:df48a3f723e5a8da61c3fe401172195f12c520a37e19a2bd4f5af03ee6f1c39e"; await buildCandidate(source); showDecision("READY", autoRun ? "Candidate built. Starting local verification…" : "Example inputs loaded and candidate built. Select RUN LOCAL GATE."); if (autoRun) global.document.getElementById("gate-form").requestSubmit(); } catch (error) { showDecision("BLOCK", error.message); } finally { button.disabled = false; if (demo) demo.disabled = false; if (!autoRun) run.disabled = false; } }
