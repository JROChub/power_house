...
require(path.join(root, "publicpower/ckodmk/browser-gate.js"));
const gate = global.CKODMKBrowserGate;

const contractText = fs.readFileSync(path.join(root, "publicpower/ckodmk/demo/browser-contract.json"), "utf8");
const contract = gate.validateContract(gate.parseStrictJson(contractText));
const datasetBuffer = fs.readFileSync(path.join(root, "publicpower/ckodmk/demo/optdigits-official-test.npz"));
