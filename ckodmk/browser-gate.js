...
(function (global) {
  ...
  async function runFromUi(event) {
    ...
    try {
      ...
      const sourceFile = chosen("gate-source"), candidateFile = chosen("gate-candidate"), datasetFile = chosen("gate-dataset"), contractFile = chosen("gate-contract"); if (!sourceFile || !candidateFile || !datasetFile || !contractFile) throw new GateError("select all four input files");
      ...
      setText("gate-changes", `${adjudication.changes.toLocaleString()} / ${contract.dataset.samples.toLocaleString()}`); setText("gate-accuracy", `${adjudication.accuracyDeltaNumerator}/${adjudication.accuracyDeltaDenominator}`); setText("gate-linf", String(observed.maxLinf)); renderClaims(adjudication.claims); showDecision(adjudication.decision, adjudication.decision === "PASS" ? "Every required browser claim passed." : adjudication.decision === "BLOCK" ? "At least one required claim failed." : "A required claim was outside this browser evidence boundary.");
      lastReport = { schema: "mfenx/ckodmk-browser-report/v1", created_at: new Date().toISOString(), authentication: "none", runtime: { name: "onnxruntime-web", version: "1.27.0", backend: "wasm", batch_size: 1 }, contract: { id: contract.contract_id, sha256: contractDigest }, artifacts: { source_sha256: sourceDigest, candidate_sha256: candidateDigest, dataset_sha256: datasetDigest }, measurements: { samples: contract.dataset.samples, class_count: contract.output.class_count, source_correct: observed.sourceCorrect, candidate_correct: observed.candidateCorrect, decision_changes: adjudication.changes, max_linf: observed.maxLinf }, raw_observation_evidence: { labels: Array.from(dataset.labels), source_decisions: observed.sourceDecisions, candidate_decisions: observed.candidateDecisions, source_outputs_float32_hex: observed.sourceBits, candidate_outputs_float32_hex: observed.candidateBits }, claims: adjudication.claims, decision: adjudication.decision, nonclaims: ["native runtime behavior", "device identity", "latency", "power", "thermal behavior", "universal equivalence"] }; download.disabled = false; const share = global.document.getElementById("share-report"); if (share) share.disabled = !(global.navigator?.share && global.navigator?.canShare?.({ files: [reportFile()] }));
    } catch (error) {
  ...
  async function loadDemo(autoRun = false) { const button = global.document.getElementById("load-demo"), demo = global.document.getElementById("run-demo"), run = global.document.getElementById("run-gate"); button.disabled = true; if (demo) demo.disabled = true; run.disabled = true; showDecision("LOADING", "Loading the retained trained model from this origin…"); try { const [source, candidate, dataset, contract] = await Promise.all([fetchFile("demo/source.onnx", "source.onnx", "application/octet-stream"), fetchFile("demo/candidate-int8.onnx", "candidate-int8.onnx", "application/octet-stream"), fetchFile("demo/optdigits-official-test.npz", "optdigits-official-test.npz", "application/octet-stream"), fetchFile("demo/browser-contract.json", "browser-contract.json", "application/json")]); setInputFile("gate-source", source); setInputFile("gate-candidate", candidate); setInputFile("gate-dataset", dataset); setInputFile("gate-contract", contract); global.document.getElementById("gate-contract-digest").value = "sha256:f03b6dc284ecd140ea9d7a95c88711079eb585482eb3993173527ceab4a18cf5"; showDecision("READY", autoRun ? "Trained model loaded. Starting local verification…" : "Trained model loaded. Select RUN LOCAL GATE."); if (autoRun) global.document.getElementById("gate-form").requestSubmit(); } catch (error) { showDecision("BLOCK", error.message); } finally { button.disabled = false; if (demo) demo.disabled = false; if (!autoRun) run.disabled = false; } }
  ...
  function bindUi() { const labels = [["gate-source", "source-name"], ["gate-candidate", "candidate-name"], ["gate-dataset", "dataset-name"], ["gate-contract", "contract-name"]]; for (const [inputId, labelId] of labels) global.document.getElementById(inputId).addEventListener("change", (event) => setText(labelId, event.target.files[0]?.name || "No file selected")); global.document.getElementById("gate-form").addEventListener("submit", runFromUi); global.document.getElementById("load-demo").addEventListener("click", () => loadDemo(false)); global.document.getElementById("run-demo").addEventListener("click", () => loadDemo(true)); global.document.getElementById("cancel-gate").addEventListener("click", () => { cancelRequested = true; }); global.document.getElementById("download-report").addEventListener("click", downloadReport); global.document.getElementById("share-report").addEventListener("click", shareReport); }

  const api = Object.freeze({ GateError, parseStrictJson, validateContract, parseStoredNpz, parseNpy, parseDataset, decimalFraction, doubleFraction, adjudicate, crc32 }); global.CKODMKBrowserGate = api; if (typeof module !== "undefined" && module.exports) module.exports = api; if (global.document) global.addEventListener("DOMContentLoaded", bindUi);
