...
(function (global) {
  ...
  async function loadDemo() { const button = global.document.getElementById("load-demo"), run = global.document.getElementById("run-gate"); button.disabled = true; run.disabled = true; showDecision("LOADING", "Loading the retained trained-model demo from this origin…"); try { const [source, candidate, dataset, contract] = await Promise.all([fetchFile("demo/source.onnx", "source.onnx", "application/octet-stream"), fetchFile("demo/candidate-int8.onnx", "candidate-int8.onnx", "application/octet-stream"), fetchFile("demo/optdigits-official-test.npz", "optdigits-official-test.npz", "application/octet-stream"), fetchFile("demo/browser-contract.json", "browser-contract.json", "application/json")]); setInputFile("gate-source", source); setInputFile("gate-candidate", candidate); setInputFile("gate-dataset", dataset); setInputFile("gate-contract", contract); global.document.getElementById("gate-contract-digest").value = "sha256:f03b6dc284ecd140ea9d7a95c88711079eb585482eb3993173527ceab4a18cf5"; showDecision("READY", "Real trained-model demo loaded. Select RUN LOCAL GATE."); } catch (error) { showDecision("BLOCK", error.message); } finally { button.disabled = false; run.disabled = false; } }
  function downloadReport() { if (!lastReport) return; const blob = new Blob([JSON.stringify(lastReport, null, 2) + "\n"], { type: "application/json" }), url = URL.createObjectURL(blob), anchor = global.document.createElement("a"); anchor.href = url; anchor.download = `ckodmk-browser-report-${Date.now()}.json`; anchor.click(); URL.revokeObjectURL(url); }
  function bindUi() { const labels = [["gate-source", "source-name"], ["gate-candidate", "candidate-name"], ["gate-dataset", "dataset-name"], ["gate-contract", "contract-name"]]; for (const [inputId, labelId] of labels) global.document.getElementById(inputId).addEventListener("change", (event) => setText(labelId, event.target.files[0]?.name || "No file selected")); global.document.getElementById("gate-form").addEventListener("submit", runFromUi); global.document.getElementById("load-demo").addEventListener("click", loadDemo); global.document.getElementById("cancel-gate").addEventListener("click", () => { cancelRequested = true; }); global.document.getElementById("download-report").addEventListener("click", downloadReport); }
