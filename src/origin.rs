...
pub const ORIGIN_RECEIPT_SCHEMA_V1: &str = "power-house/origin-receipt/v1";

const CAPACITY_DOMAIN: &[u8] = b"power-house:origin:v1:creative-capacity\0";
const RECEIPT_DOMAIN: &[u8] = b"power-house:origin:v1:creation-receipt\0";
...
impl CreationReceipt {
    fn new(
        ...
        let mut receipt = Self {
            schema: ORIGIN_RECEIPT_SCHEMA_V1.to_string(),
            label,
            parent_rootprint_id,
    ...
    pub fn verify(&self) -> Result<(), OriginError> {
        if self.schema != ORIGIN_RECEIPT_SCHEMA_V1 {
